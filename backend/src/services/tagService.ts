...
// 关注标签（幂等）：仅当新建关注时 Tag.followCount + 1
export async function followTag(userId: number, tagName: string): Promise<void> {
...
// 取消关注标签：仅当存在关注记录时 Tag.followCount - 1
export async function unfollowTag(userId: number, tagName: string): Promise<void> {
  await prisma.$transaction(async (tx) => {
