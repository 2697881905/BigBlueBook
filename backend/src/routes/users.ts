...
router.delete('/:id/follow', auth, async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/:id/following', auth, async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/:id/followers', auth, async (req: AuthRequest, res: Response) => {
  try {
    const page = req.query.page ? Number(req.query.page) : 1;
    const limit = req.query.limit ? Number(req.query.limit) : 20;
